...
CREATE TABLE `guarantee` (
  ...
  `order_no` int(11) NOT NULL COMMENT '中鑫中科订单号',
...
CREATE TABLE `product` (
  ...
  `product_id` varchar(64) NOT NULL COMMENT '省综合服务平台产品id标识',
...
CREATE TABLE `application` (
    `id` bigint(20) not null AUTO_INCREMENT comment '主键id',
    `productId` varchar(100) default null comment '省综合服务平台产品id标识',
    `companyName` varchar(50) default null comment '企业名称',
    `companyCreditCode` varchar(50) default null comment '企业统一社会信用代码',
    `companyAddress` varchar(500) default null comment '企业地址',
    `handlerName` varchar(50) default null comment '经办人姓名',
    `handlerMobile` varchar(20) default null comment '经办人手机号',
  `create_time` timestamp NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` timestamp NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `delete_flag`  tinyint(1) NULL DEFAULT NULL COMMENT '逻辑删除（0 未删除、1 删除）',
    PRIMARY KEY (`id`)
...
CREATE TABLE `invoice` (
    `id` bigint(20) not null AUTO_INCREMENT comment '主键id',
    `orderNo` varchar(100) default null comment '中鑫中科订单号',
    `guarantee` varchar(100) default null comment '保函编号',
    `invoiceUrl` varchar(255) default null comment '发票下载地址，专票可不传',
    `invoiceCode` varchar(50) default null comment '发票代码',
    `invoiceNo` varchar(50) default null comment '发票号码',
    `applyStatus` tinyint(1) default '0' comment '是否申请成功 1：审核通过 0：审核不通过',
    `applicantName` varchar(50) default null comment '投保人名称',
    `applicantCode` varchar(50) default null comment '投保人统一社会信用代码',
    ...
    `contactMobile` varchar(20) default null comment '联系电话（收件人联系电话，专票时必填）',  
    `contactName` varchar(50) default null comment '收件人联系电话姓名，专票时必填',
    `contactAddress` varchar(500) default null comment '收件人地址，专票时必填',
      `create_time` timestamp NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` timestamp NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `delete_flag`  tinyint(1) NULL DEFAULT NULL COMMENT '逻辑删除（0 未删除、1 删除）',
	PRIMARY KEY (`id`)
